...
function mejoresPartidasGeneral(turno,disparoSiguiente,lanzaderas,vulnerabilidades,barcos){
    ...
    for (let a of alternativas){
        if(mejores.length==0 || mejorPuntaje>a.mejorPuntajePosible(vulnerabilidades(turno))){
            mejores=[a]
            mejorPuntaje=a.mejorPuntajePosible(vulnerabilidades(turno))
        }else if(mejorPuntaje==a.mejorPuntajePosible(vulnerabilidades(turno))){
            mejores.push(a)
        }
    }
    return mejores
